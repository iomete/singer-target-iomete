    ...
    for line in lines:
        ...
        if t == 'RECORD':
            ...
            if stream not in schemas or schemas[stream] != new_schema:
                ...
                #  1) Set ` 'primary_key_required': false ` in the singer_target_iomete config.json
                #  or
                #  2) Use fastsync [postgres-to-iomete, mysql-to-iomete, etc.]
...
def flush_streams(
    ...
    :param stream_to_sync: iomete db sync instance per stream
...
def flush_records(stream: str,
    ...
    """
    Takes a list of record messages and loads it into the iomete target table
    ...
    filepath = csv_format.records_to_file(records,
    ...
    # Upload to s3 and load into iomete
